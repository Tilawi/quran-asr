...
export interface QuranChampionMatch {
    ...
    raw_score: number;
    bonus: number;
    /** score() gap to the best truly-different candidate (weak signal for fragments). */
    margin?: number;
    /** pure semi-global fragment fit of the query against this match's phonemes. */
    frag_fit?: number;
    /** frag_fit gap to the best truly-different candidate — the fragment discriminator. */
    fit_margin?: number;
...
/** Row of the lazily-built global span table (see `_jointGlobalSpanTable`). */
interface JointGlobalSpanRow {
    surah: number;
    ayah: number;
    ayah_end: number;
    phonemes: string;
...
export class QuranDB {
    verses: QuranVerse[];
    private _byRef = new Map<string, QuranVerse>();
    private _bySurah = new Map<number, QuranVerse[]>();
    private _jointPrefixSpans: QuranChampionMatch[] | null = null;
    private _jointGlobalSpans: JointGlobalSpanRow[] | null = null;
            ...
            if (v.ayah === 1 &&
                ...
                v.phoneme_token_ids_no_bsm =
                    bsmTokenEnd > 0 && (v.phoneme_token_ids?.length ?? 0) > bsmTokenEnd
                        // Non-null assertion: the guard above implies phoneme_token_ids is defined.
    ...
    private _startsWithArabicBismillah(words: string[] | undefined): boolean {
    ...
    search(text: string, topK = 5): (QuranVerse & {
        score: number;
    })[] {
    ...
    retrieveCandidates(text: string, { maxSpan = 4, hint = null, singleLimit = 32, topSurahs = 3, spanLimit = 32, }: RetrievalOptions = {}): CandidateRetrieval {
        if (!text.trim()) {
        ...
        const singles = scored
            .slice(0, singleLimit)
            .map(([v, raw, bonus, total]) => this._candidateFromVerse(v, raw, bonus, total));
    ...
    matchVerse(text: string, threshold = 0.3, maxSpan = 3, hint: [number, number] | null = null, returnTopK = 0): QuranChampionMatch | null {
    ...
    bestJoint03Match(text: string): QuranChampionMatch | null {
        ...
        // Selection logic below is unchanged; it only assigns `winner` at the
        // former return points so a single exit can annotate confidence.
        let winner: QuranChampionMatch | undefined;
        ...
        if (shortOpening) {
            ...
            if (!bestIsLateSpan && !lowConfidence) {
                ...
                if (noSpaceLen >= JOINT_OPENING_COLLAPSE_MIN_CHARS &&
                    noSpaceLen <= JOINT_OPENING_COLLAPSE_MAX_CHARS &&
                    best.ayah_end != null &&
                    best.ayah > 1) {
                    const sameSurahPrefix = prefix
                        .filter((p) => p.surah === best.surah &&
     ...
     */
    private _annotateConfidence(winner: QuranChampionMatch, top: QuranChampionMatch[], text: string): void {
        const noSpaceText = text.replace(/ /g, "");
        const refNs = (winner.phonemes_joined ?? "").replace(/ /g, "");
        const winnerFit = refNs && noSpaceText ? fragmentScore(noSpaceText, refNs) : 0;
        winner.frag_fit = QuranDB._round4(winnerFit);
        // Best score and best fragment-fit among TRULY DIFFERENT candidates (not
        // the winner and not a same-surah span that overlaps it — an overlapping
        // span is the same answer, not a rival). Score-margin is unreliable for
        // fragments: a short verse with coincidental whole-string overlap scores
        // high but fits poorly. Fit-margin is the decisive discriminator.
        ...
        winner.margin = QuranDB._round4(winner.score - bestOtherScore);
        winner.fit_margin = QuranDB._round4(winnerFit - bestOtherFit);
    }
    /** True when candidate c is the winner or a same-surah span that overlaps it. */
    private _sameOrOverlapping(c: QuranChampionMatch, w: QuranChampionMatch): boolean {
        if (c.surah !== w.surah)
            return false;
    ...
    private _joint02MatchPhonemeText(phonemeText: string, topK = JOINT_TOP_K_LEVENSHTEIN): QuranChampionMatch[] {
        if (!phonemeText.trim())
            return [];
        ...
        const singles = scored.slice(0, Math.max(topK, 32)).map(([verse, raw, boosted]): QuranChampionMatch => {
    ...
    private _jointGlobalSpanCandidates(phonemeText: string): QuranChampionMatch[] {
        ...
        const rough: [number, JointGlobalSpanRow][] = [];
        for (const row of this._jointGlobalSpanTable()) {
            const ov = QuranDB._intersectionSize(qb, row.bigrams) +
    ...
    private _jointShortOpeningSpanCandidate(phonemeText: string, bestScore: number): QuranChampionMatch | null {
        const noSpaceText = phonemeText.replace(/ /g, "");
        ...
        for (const row of this._jointPrefixSpanTable()) {
            ...
            if (!first || !second)
                continue;
            const firstNs = first.phonemes_joined_no_bsm_ns ??
            ...
            const frag = fragmentScore(noSpaceText, row.phonemes_joined.replace(/ /g, ""));
            const score = Math.max(raw, raw + (frag - raw) * JOINT_FRAGMENT_BLEND);
            if (score >= JOINT_SHORT_OPENING_MIN_SCORE &&
    ...
    private _jointCandidateVerses(noSpaceText: string, maxCandidates = 950): QuranVerse[] {
        ...
        for (let i = 0; i < this.verses.length; i++) {
            ...
            if (refNs.length < 2)
                continue;
            const ov = QuranDB._intersectionSize(qb, QuranDB._jointNgrams(refNs, 2)) +
    ...
    private _jointGlobalSpanTable(): JointGlobalSpanRow[] {
        if (this._jointGlobalSpans)
            return this._jointGlobalSpans;
        const spans: JointGlobalSpanRow[] = [];
        for (const [surahNum, verses] of this._bySurah.entries()) {
            for (let i = 0; i < verses.length; i++) {
    ...
    private _candidateFromVerse(verse: QuranVerse, raw: number, bonus: number, total: number): QuranCandidate {
        return {
            surah: verse.surah,
            ayah: verse.ayah,
            ayah_end: verse.ayah,
    ...
    private _candidateFromSpan(chunk: QuranVerse[], raw: number, bonus: number, total: number, surahRank: number): QuranCandidate {
        const first = chunk[0];
    ...
    private _shortQueryBoost(noSpaceText: string, verse: QuranVerse, useNoBsm = false): number {
